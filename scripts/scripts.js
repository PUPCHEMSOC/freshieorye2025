...
function generateTicket() {
    ...
    if (session === 'Morning Session') {
        sessionTitle = 'Initium Fati';
        sessionImage = 'https://i.imgur.com/et97avL.png';
...
// Insert image instead of text
    ...
    `<img src="${sessionImage}" alt="${sessionTitle}" style="max-width: 300px;">`;
...
function sendToGoogleSheet(name, section, session, referenceCode) {
    ...
    fetch('https://script.google.com/macros/s/AKfycbxryvLDMmpfKjPtblJMUzkO_6jM0BLVUvkTuAUNUfjm085tBYSJfEEw4RSPOQHtMBY/exec', {
        method: 'POST',
        body: formData,
    })
    .then(response => {
